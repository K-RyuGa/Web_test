...
def run_post_game_analysis():
    evaluation_prompt = '''あなたは、日本語学習者の会話を分析し、添削と評価を行う、プロの日本語教師です。
        ...
        **[添削のルール]**
        ...
        2.  ただし、学習者（「ユーザー:」で始まる行）の発言に文法的な誤りや不自然な表現を見つけた場合、その行だけを以下の【添削フォーマット】に従って書き換えてください。
        ...
        *   **修正した行の直後に、括弧（）を使って、修正した理由**を論理的（文法上のルールに基づく）かつ詳細に付け加えてください。
        ...
        **（添削の実行例）**
        ...
        あなたの出力（ステップ1）：
        Ｔ：どんな町ですか
        Ｉ：小さいまちです、でもとても[きれいな][きれい]町です （「きれい」は「な形容詞」なので、名詞の前では「な」が必要です。）
