...
def check_mission_status(conversation_log, agent_prompt):
    ...
    checker_prompt = f"""
        あなたは日本語学習ゲームの「審判」です。
        あなたの唯一の役割は、与えられた[ゲームのルール]と[会話の履歴]を厳密に分析し、プレイヤーがミッションを達成したかどうかを判定することです。
        ...
        [判定ルール]
        1.  **完全一致の原則**: [ゲームのルール]に記載されている「ミッション達成の条件」の**すべての項目が、会話の履歴の中で明確に満たされているか**を評価してください。一つでも欠けている場合は「継続」です。
        2.  **時系列の考慮**: 条件は会話の自然な流れに沿って満たされるべきです。例えば、「AIの説明をプレイヤーが理解した」という条件は、AIが何かを説明した**後**にのみ成立します。
        3.  **厳密な判定**: 少しでも達成条件を満たしているか曖昧な場合は、安全策として「継続」と判断してください。早すぎる判定は避けてください。
        4.  条件が**完全に**満たされていると確信した場合にのみ、**「達成」** という単語だけを出力してください。
        5.  それ以外の場合は、すべて**「継続」** という単語だけを出力してください。
        6.  あなたの出力はプログラムで自動処理されるため、「達成」「継続」以外のいかなる文字（挨拶、説明、句読点など）も絶対に出力してはいけません。

        【判定ミスの例】
        ルール: 「プレイヤーが探している商品の場所を質問し、AIの説明をプレイヤーが理解したことを示す応答（例：「わかりました」）をすること。」
        悪い判定: プレイヤーが質問しただけで「達成」としてしまう。
        正しい判定: AIが場所を説明し、その後にプレイヤーが理解していそうな応答をするまで「継続」とする。
    """
...
3.  間違いのない行、および「AI:」で始まる行は、絶対に書き換えず、そのまま出力してください。
以下出力例””
ユーザー: 野球をして[います][いまず] （動詞の活用形の誤りです。「います」が正しいです。）
AI: 野球をしているんだね！どのポジションをやっているの？試合とかもあるの？
ユーザー: [ピッチャーをしています][ぴちゃあ]（名詞のままだと文にならないため、「〜をしています」などの動詞を加えて文にしましょう。）
AI: ピッチャーなんだ！すごいね。試合のときは緊張する？それとも楽しい？
””
