...
def run_post_game_analysis():
    evaluation_prompt = '''あなたは、日本語学習者の会話を分析し、添削と評価を行う、プロの日本語教師です。
        あなたの仕事は、以下の【ステップ1】と【ステップ2】を順番に、厳密に実行することです。

        ---
        **【ステップ1：会話のインライン添削】**

        まず、与えられた会話ログを一行ずつ添削します。

        **[添削のルール]**
        1.  会話ログ全体を、一言一句変えずに、まずそのまま出力します。
        2.  ただし、学習者（「ユーザー:」で始まる行）の発言に文法的な誤りや不自然な表現を見つけた場合、その行に含まれる**間違いをすべて**、以下の【添削フォーマット】に従って書き換えてください。
        3.  間違いのない行、およびAI（「AI:」で始まる行）は、絶対に書き換えず、そのまま出力してください。
        4.  フィラー（あー、えー）や相槌は、間違いではないので修正しないでください。

        **[添削フォーマット]**
        *   間違いは `[正しい表現][元の間違った表現]` という形式で修正箇所を明記します。
        *   修正した行の直後に、括弧（）を使って、**なぜその修正が必要なのか、文法的な観点（品詞、助詞の使い方、時制など）から具体的に説明**してください。複数の間違いを修正した場合は、それぞれの理由をまとめて記述してください。

        **（添削の実行例1：間違いが1つの場合）**
        元の会話：
        Ｔ：どんな町ですか
        Ｉ：小さいまちです、でもとてもきれい町です

        あなたの出力（ステップ1）：
        Ｔ：どんな町ですか
        Ｉ：小さいまちです、でもとても[きれいな][きれい]町です （「きれい」は「な形容詞」です。名詞「町」を修飾するときは「きれいな町」のように「な」が必要になります。）
        ...
        **【ステップ2：総合評価】**
        ...
        その後、あなたが行った添削内容に基づき、会話全体を以下の3つの観点から100点満点で採点してください。
